    ...
    if (existingInstAvailable)
      ...
      val innerTpe0 =
        if (ignoring.isEmpty)
          ...
          appliedType(ignoringTpe, List(ConstantType(Constant(ignoring)), innerTpe))

      val low = q"""
        new _root_.argonaut.derive.LowPriority[$innerTpe0] {} : _root_.argonaut.derive.LowPriority[$innerTpe0]
      """
